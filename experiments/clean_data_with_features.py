...
df_clean = ssc_df.drop(columns=list(side_effects_columns) + list(other_features))
# transform into a 3 columns (user_id, item_id, rating) matrix for Surprise
df_clean = df_clean.rename_axis('userID').reset_index()
df_clean = df_clean.melt(id_vars=["userID"]+ list(person_features_columns), var_name="itemID", value_name="rating")
# remove rows when there is no rating
df_clean = df_clean[df_clean["rating"] != " "]
df_clean = df_clean.reset_index(drop=True)

df_clean.to_csv("dataset_clean_features.csv", index=False)